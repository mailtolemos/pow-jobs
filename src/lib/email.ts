...
export function renderDigestHTML(input: DigestInput): string {
    ...
    .map(({ match, job }) => {
      ...
      <div style="border: 1px solid #E5E5E0; border-radius: 12px; padding: 20px; margin-bottom: 16px; background: #ffffff;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
          ...
          <div style="font-size: 13px; font-weight: 600; color: #1F3A5F; background: #EEF3FB; padding: 4px 10px; border-radius: 999px; white-space: nowrap;">${scorePct}% match</div>
        </div>
        ...
        <a href="${escapeHTML(job.source_url)}" style="display: inline-block; font-size: 13px; font-weight: 600; color: #ffffff; background: #1F3A5F; padding: 8px 14px; border-radius: 8px; text-decoration: none;">View role</a>
      </div>`;
    })
    .join("");
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Your weekly PoW Jobs digest</title></head>
<body style="margin: 0; padding: 0; background: #FAFAF7; font-family: -apple-system, 'Segoe UI', Roboto, Inter, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 32px 20px;">
    <div style="font-size: 11px; letter-spacing: 0.1em; color: #5A6578; text-transform: uppercase; margin-bottom: 8px;">PoW Jobs · Week of ${escapeHTML(weekOf.slice(0, 10))}</div>
    ...
    <div style="border-top: 1px solid #E5E5E0; margin-top: 24px; padding-top: 16px; font-size: 12px; color: #8A94A6;">
      You're receiving this because you opted in to weekly matches. <a href="#" style="color: #1F3A5F;">Update preferences</a> · <a href="#" style="color: #1F3A5F;">Pause for 4 weeks</a> · <a href="#" style="color: #1F3A5F;">Unsubscribe</a>
    </div>
  </div>
</body>
</html>`;
...
<body style="margin: 0; padding: 0; background: #FAFAF7; font-family: -apple-system, 'Segoe UI', Roboto, Inter, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 32px 20px;">
    ...
    <a href="#" style="display: inline-block; font-size: 13px; font-weight: 600; color: #ffffff; background: #1F3A5F; padding: 10px 16px; border-radius: 8px; text-decoration: none;">Adjust preferences</a>
  </div>
</body>
</html>`;
}
